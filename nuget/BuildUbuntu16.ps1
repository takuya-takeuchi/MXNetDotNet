...
$MXNetDotNetRoot = Split-Path $ScriptPath -Parent
$NugetPath = Join-Path $MXNetDotNetRoot "nuget" | `
...
$MXNetDotNetRoot = (Split-Path (Get-Location) -Parent)
$MXNetDotNetSourceRoot = Join-Path $MXNetDotNetRoot src
...
foreach($BuildTarget in $BuildTargets)
{
   ...
   if ($target -ne "cuda")
   {
      ...
      $dockername = "mxnetdotnet/build/$Distribution/$DistributionVersion/$Target" + $postfix
      $imagename  = "mxnetdotnet/devel/$Distribution/$DistributionVersion/$Target" + $postfix
   }
   else
   {
      $option = $cudaVersion
      ...
      $dockername = "mxnetdotnet/build/$Distribution/$DistributionVersion/$Target/$cudaVersion"
      $imagename  = "mxnetdotnet/devel/$Distribution/$DistributionVersion/$Target/$cudaVersion"
   }

   $Config = [Config]::new($MXNetDotNetRoot, "Release", $target, $architecture, $platform, $option)
   ...
   {
      Write-Host "Start 'docker run --rm -v ""$($MXNetDotNetRoot):/opt/data/MXNetDotNet"" -e LOCAL_UID=$(id -u $env:USER) -e LOCAL_GID=$(id -g $env:USER) -t $dockername'" -ForegroundColor Green
      docker run --rm `
                  -v "$($MXNetDotNetRoot):/opt/data/MXNetDotNet" `
   ...
   foreach ($key in $BuildSourceHash.keys)
   {
      $srcDir = Join-Path $MXNetDotNetSourceRoot $key
